...
# Stage 6:
## Displaying priority-based unread notifications in frontend

We have  to show notifications based on priority:
 **Placement > Result > Event**.
Also, we only show **top 10 unread notifications**, and we track the read state using **local browser storage** so we don't have to keep calling the server to mark them as read.

### Challenges and requirements:
1. **Priority Sorting**: Notifications should be sorted by their type where `placement` is highest priority, then `result`, and then `event`.
2. **Local Storage Read State**: Since we might not want to hit the server database every time for minor read updates, we can save the list of read notification IDs in localStorage (e.g. `read_notifications`).
3. **Filtering & Top 10 limit**: Filter out notifications that are in the read list and then only take the first 10 (or top `N` notifications like 10, 15, 20 based on configuration).

### How to do this in Frontend?

##### 1. Fetching notifications from API
* Call `GET /notifications` using `REACT_API_KEY` and pass the `AUTH_TOKEN` in headers as `Authorization: Bearer <token>`.

##### 2. LocalStorage Syncing
* On component mount, load read IDs from `localStorage.getItem('read_notifications')`.
* When user clicks on a notification, add its ID to the list and save back to localStorage.

##### 3. Frontend Sorting and Filtering logic
* Filter notifications where `status != 'read'` and its ID is not present in our local storage read list.
* Map type to priority values: `placement -> 3`, `result -> 2`, `event -> 1`.
* Sort first by priority (descending), and if priority is same, sort by `created_at` (newest first).
* Use `.slice(0, N)` to show only the top `N` notifications.
...
export function useNotifications(limit = 10) {
  ...
  useEffect(() => {
    const fetchNotifs = async () => {
      setLoading(true);
      try {
        const response = await fetch(import.meta.env.REACT_API_KEY, {
          headers: {
        ...
        // Ensure we handle different API formats
  ...
  // Save read IDs to localStorage whenever it changes
  useEffect(() => {
  ...
  const markAllRead = () => {
    ...
    const newReadIds = [...readIds, ...unreadFiltered.map(n => n.id)];
    setReadIds(newReadIds);
  };

  // Filter, Sort and Get Top N
  const displayNotifications = notifications
    .filter(n => n.status !== 'read' && !readIds.includes(n.id))
    .sort((a, b) => {
      const priorityA = PRIORITY_MAP[a.type?.toLowerCase()] || 0;
      const priorityB = PRIORITY_MAP[b.type?.toLowerCase()] || 0;
      
      if (priorityA !== priorityB) {
        return priorityB - priorityA; // Placement > Result > Event
      }
      // If priority is same, sort by latest date
